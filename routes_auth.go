...
func (s *server) handleTokenCreate() http.HandlerFunc {
	...
	return func(w http.ResponseWriter, r *http.Request) {
		...
		if err != nil {
			...
			log.Println(msg)
			s.respond(w, r, responseError{Error: msg}, http.StatusBadRequest)
		}
		...
		if err != nil {
			...
			s.respond(w, r, responseError{Error: msg}, http.StatusInternalServerError)
			return
		}

		if !found {
			s.respond(w, r, responseError{Error: "Invalid credentials"}, http.StatusBadRequest)
			return
		...
		s.respond(w, r, response{Token: tokenStr}, http.StatusOK)
	}
}
